...
class ModifiedIterativeAmplitudeEstimation(AmplitudeEstimator):
    ...
    def __init__(
        ...
        Args:
            ...
            shots: Number of shots of circuit to run at one time
        ...
        if shots <= 0:
            raise ValueError(f"The number of shots must be nonnegative, but is {shots}")
    ...
    def estimate(
        ...
        # AR modification
        # having shots as a parameter to function causes problems in my code, it is now a class member but
        # allows a copy to be modified here
        shots = self._shots
            ...
            while powers[num_iterations - 1] == k:
                ...
                if self._confint_method == "beta":
                    ...
                    if self._check_early(shots, alpha_i, nu, theta_intervals, self._epsilon):
                        if round_shots < shots_i_max:
                            N = min(round_shots + shots, shots_i_max)
                        else:
                            N = round_shots + 1

                        shots_to_run = N - round_shots
                ...
                if verbose:
                    ...
                    print('N:', N)
                    print('shots to run:', shots_to_run)

                # run measurements for Q^k A|0> circuit
                circuit = self.construct_circuit(estimation_problem, k, measurement = True)
                
                try:
                    job = self._sampler.run([circuit], shots = shots_to_run)
                    ret = job.result()
                except Exception as exc:
                    raise AlgorithmError("The job was not completed successfully.") from exc
                
                # retrieve counts of outcomes
                counts = { k: round(v * shots_to_run) for k, v in ret.quasi_dists[0].binary_probabilities().items() }
...
#                 # bookkeeping
            if 'round_shots' not in state: state['round_shots'] = {}
            state['round_shots'][k] = N
            if 'n_queries' not in state: state['n_queries'] = {}
